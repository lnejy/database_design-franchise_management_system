 ...
 * 
 * 담당 테이블: store, store_inventory, ingredient, store_material_request
 * 
 * 주요 기능:
 * - 매장 로그인 인증
 * - 매장 정보 조회 및 등록
...
public class StoreDAO {
     ...
     * 
     * DB 흐름:
     * 1. store 테이블에서 store_code와 phone_number로 매장 정보 조회
     * 2. 일치하는 매장이 있으면 StoreDTO 반환, 없으면 null 반환
     * 
     ...
     * 
     * DB 흐름:
     * 1. store 테이블에서 store_code로 매장 정보 조회
     * 2. 매장 정보를 StoreDTO로 변환하여 반환
     * 
     * @param id 매장 코드
     ...
     * 
     * DB 흐름:
     * 1. store 테이블에 새로운 매장 정보 INSERT
     * 2. 성공 시 true, 실패 시 false 반환
     * 
     * 주의사항: store_code는 UNIQUE 제약조건이 있어야 합니다.
     * 
     * @param dto 등록할 매장 정보
     * @return 등록 성공 여부
     */
     ...
     * 모든 매장 목록 조회
     * 
     * 사용처: 키오스크 시작 화면에서 매장 선택
     * 
     * DB 흐름:
     * 1. store 테이블에서 모든 매장 정보 조회
     * 2. StoreDTO 리스트로 반환
     * 
     * @return 모든 매장 정보 리스트
     */
     ...
     * 2. 해당 매장의 재료명, 수량, 단위, 카테고리 조회
     * 3. 카테고리와 재료명 순으로 정렬
     * 
     * 반환 형식: JTable에 직접 사용 가능한 Vector<Vector<Object>>
     * 
     * @param storeId 매장 ID
     ...
     * 모든 재료 목록 조회
     * 
     * 사용처: 발주 다이얼로그의 재료 선택 콤보박스
     * 
     * DB 흐름:
     * 1. ingredient 테이블에서 모든 재료 정보 조회
     * 2. 재료명 순으로 정렬
     * 3. IngredientDTO 리스트로 반환
     * 
     * @return 모든 재료 정보 리스트
     */
     ...
     * 
     * DB 흐름:
     * 1. store_material_request 테이블에 발주 요청 INSERT
     * 2. status는 'PENDING'으로 설정 (본사 승인 대기 상태)
     * 3. request_date는 자동으로 현재 시간 설정
     * 
     * 다음 단계: 본사 관리자가 HQMainView에서 승인/반려 처리
     * 
     * @param storeId 매장 ID
